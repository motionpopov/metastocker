...
def review(posts, output, host=None, fresh_sources=None):
    ...
    schema = {'type':'object','properties':{'articles':{'type':'array','minItems':len(posts),'maxItems':len(posts),'items':{'type':'object','properties':{'slug':{'type':'string','enum':[post['slug'] for post in posts]},'approved':{'type':'boolean'},'issues':{'type':'array','items':{'type':'string'}}},'required':['slug','approved','issues'],'additionalProperties':False}}},'required':['articles'],'additionalProperties':False}
